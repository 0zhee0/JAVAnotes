...
public class Ex2 {
	...
	public static void main(String[] args) {
		 ...
		 * - 대부분의 생서자 및 메서드가 deprecate 처리되어 있음
		...
		// 생성자 long 타입 값을 전달 시 해당 값에 맞는 Date 객체 생성됨
//		Date d2 = new Date(0); // 1970년 1월 1일 09시 기준 설정
		...
		// => 두 Date 객체의 getTime() 결과를 연산하면 두 날짜 간의 차이 계산 가능
		...
		System.out.println(differentDate / 1000 / 60 / 60 / 24 + "일"); // 1000을 나누면 ms -> s 여기서 60으로 나누면 s(초) -> m(분)
																		// 또 60으로 나누면 m(분) -> h(시) 여기서 24로 나누면 h(시) -> d(일)
		if(differentDate > 0) {
