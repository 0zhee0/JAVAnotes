...
public class Ex4 {
	...
	public static void main(String[] args) {
		/*
		 * java.time 패키지
		 * - 날짜 및 시각 정보를 관리하는 기능의 패키지로 JDK 8 부터 제공됨
...
//		int month = date.getMonth();
		...
		System.out.println(year + "/" + month +"/" + day); // 원래 toString 형태로 날짜 출력하면 '-'로 나뉘는데 "/" 로 나누고 싶어서 날짜 값을 따로 분리 후 입력함
		
		System.out.println("-------------Month 타입 객체 활용-------------");
		
		// Month 타입 객체 활용
		// => enum 타입 객체인 Month 타입으로 관리됨(각 월의 이름이 상수로 제공)
		Month enumMonth = date.getMonth();		// class a / interface a 처럼 enum a; 타입이 존재
		System.out.println(enumMonth); // toString() 생략됨
		String monthName = enumMonth.toString(); // 변수 저장 시 String 변환은 필수!
		
		// Month 객체를 한국식 표현법으로 변환할 경우
		System.out.println(enumMonth.getDisplayName(TextStyle.SHORT, Locale.ENGLISH));
		System.out.println(enumMonth.getDisplayName(TextStyle.FULL, Locale.ENGLISH)); // FULL로 할 경우 full 스펠링 출력
		System.out.println(enumMonth.getDisplayName(TextStyle.SHORT, Locale.KOREAN));  // F3 누르면 내장된 해당 객체가 담겨있는 파일로 이동
		...
		//	  같으면 0 리턴함
		System.out.println(date3.compareTo(date4));		// -22
