...
//		Calendar cal = new Calendar(); // 추상클래스이므로 인스턴스 생성 불가
		...
		System.out.println(cal); // java.util.GregorianCalendar[time=1666575276853,areFieldsSet=true,areAllFieldsSet=true,lenient=true,zone=sun.util.calendar.ZoneInfo[id="Asia/Seoul",offset=32400000,dstSavings=0,useDaylight=false,transitions=30,lastRule=null],firstDayOfWeek=1,minimalDaysInFirstWeek=1,ERA=1,YEAR=2022,MONTH=9,WEEK_OF_YEAR=44,WEEK_OF_MONTH=5,DAY_OF_MONTH=24,DAY_OF_YEAR=297,DAY_OF_WEEK=2,DAY_OF_WEEK_IN_MONTH=4,AM_PM=0,HOUR=10,HOUR_OF_DAY=10,MINUTE=34,SECOND=36,MILLISECOND=853,ZONE_OFFSET=32400000,DST_OFFSET=0]
		...
		//	  Calendar 객체가 존재하는 상태에서 setTime() 메서드 호출하여
		//	  Date 타입 객체를 파라미터로 전달
		Calendar cal2 = Calendar.getInstance(); // 추상클래스인 Calendar를 인스턴스화하는 방법
		cal2.setTime(date);
		...
		// 각 항목을 별도로 변경할 때는 ser(int field, int value) 메서드 사용
		// => field 항목은 get() 메서드와 마찬가지로 Calendar.XXX 상수 사용
		cal2.set(Calendar.YEAR, 2010); // 2010년을 설정
