...
public class Ex2 {
	...
	public static void main(String[] args) {
		/*
		 * if ~ else 문
		 * - 조건문 판별 결과가 true 일 때와 false 일 때 서로 다른 블록을 실행
		 * - 삼항연산자와 유사한 형태로 수행되나 삼항연산자보다 유연함
		 * 	 (조건식 판별 결과에 따라 실행 가능한 문장이 더 다양함)
		 * - if 문 블록은 조건식 결과가 true  일 때 실행하고,
		 * 	 else문 블록은 조건식 결과가 flase 일 때 실행함.
		 * 	      
		 * 
		 * <기본 문법>
		 * 문장1;
		...
		if(num2 % 2 == 1) { // 홀수일 경우 // num2 % 2 > 0과 동일한 조건식
			// "num은 홀수!" 출력
			System.out.println( num2 + " : 홀수!" );
		} else {
			// 홀수가 아닐 경우 = 짝수(또는 0)일 경우
			// "num은 짝수!" 출력
