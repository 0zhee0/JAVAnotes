...
public class Ex3 {
	...
	public static void main(String[] args) {
		/*
		 * 다중 if - else if 문(if ~ else if ~ else 문)
		 * - 두 가지 이상의 조건식을 사용하여 세 가지 이상의 경우의 수 판별
		 * - 기본 if 문으로 첫번째 조건을 설정하고, else if 문으로 
		 *   두번째 이후의 조건을 설정한다.
		 * - 모든 조건이 만족하지 않을 때 실행할 문이 있을 경우
		 *   문장의 마지막에 else 문 사용
		 * - if 문 비교는 위에서 아래로 차례대로 비교를 수행한다.
		 ...
		 * 		// 조건식1의 결과가 true 일 때
		 * } else if(조건식2) {
		 * 		// 조건식 1의 결과가 false 이고, 조건식 2의 결과가 true 일 때
		 * } else if(조건식n) {
		 * 		// 조건식 1 ~ n-1 의 결과가 false 이고, 조건식 n의 결과가 true 일 때
		 * } else {
		...
		if(num > 5) {
			...
			// num > 10 조건식은 실행될 수 없는 문장이다!
			System.out.println(num + " : 10보다 크다");
		}
		
		// 만약, 6 ~ 9의 숫자로 판단하려면?
		// else if 문 작성 시에는 조건식의 범위가 좁은 조건식부터 판별해야함
		if(num > 10) {
			System.out.println(num + " : 10보다 크다");
		} else if(num > 5) {
			System.out.println(num + " : 5보다 크다"); // 6, 7, 8, 9 중의 숫자
		}
