...
public class Ex {
	...
	public static void main(String[] args) {
		 ...
		 *    강제로 형변환을 수행해야함(형변환 연산자에는 좌변의 작은 데이터타입명 명시)
		 *    => '(작은 데이터타입명)우변의 데이터' 형식으로 지정
		 *  - 강제 형변환으로 인해 오류가 없어지더라고
		 *    형변환 후의 데이터는 원본 데이터와 다를 수 있음( =Overflow의 경우)
...
//		short b2 = int b1 // 오류발생
		...
		int c1 = 100;
//		short c2 = c1; // 오류 발생 : c2 < c1 이라서 Overflow 오류 => 강제 형변환 필수!
		short c2 = (short)c1; // 강제 형변환 방법
		System.out.printf("변환 후 c1(int) : %d, c2(short) : %d\n", c1, c2);
		// => short 타입에 저장 가능한 범위 내의 데이터이기 때문에
		//	  명시적 형변환 후에도 Overflow 발생이 없으므로 원본 데이터가 유지 됨 
		//	  (무조건 데이터가 바뀌는 건 아님)
...
//		e1 = e2; // 오류 발생!
		...
		// long타입(88byte) vs float타입(48byte)
		long l = 100L;
		float f = l; // long < float, long -> float 변환 시 자동형변환 일어남
		
//		l = f; //  오류 발생! float -> long 변환 시 자동형변환 불가!
		l = (long)f; // 형변환 연산자(long)를 통해 long 타입으로 강제형변환 필수!
