...
public class Ex {
	...
	public static void main(String[] args) {
		 ...
		 * 	  문자열이 끝난 후 콤마(,) 기호 뒤에 형식 지정 문자에 들어갈 데이터를 각각 명시함
		...
		System.out.print(1);
		System.out.println(); // 줄바꿈 수행(엔터키역할)
//		System.out.print(); // ()안에 반드시 데이터를 입력해야한다 그렇지 않으면 오류발생
		System.out.println("----------------printf------------------"); // "----" 출력 후 줄바꿈
		
		System.out.printf("Hello, World!");
		System.out.printf("강진석");
//		System.out.printf(1);
		// 오류 발생 The method printf(String, Object...) 
		// in the type PrintStream is not applicable for the arguments (int)
		
		// 형식 지정 문자를 사용한 printf() 메서드
		// &d : 10진수 1개 표현
		// &xd : x 자릿수만큼의 자리를 차지
		// %0xd : x 자릿수만큼의 자리를 차지하고, 빈자리는 0으로 채움
		// => ex) ...printf("%2d",2) -> 02
		System.out.printf("%d",1); 
		System.out.println(); // 줄바꿈
		System.out.printf("나이는 %d세 입니다. \n", 20); // \n 은 줄바꿈을 하고자 하는 곳에 넣어주면 된다.
		System.out.printf("나이는 %d세 입니다.", age); // 숫자 대신 변수 넣어도 가능
		
		System.out.println(); // 줄바꿈
		System.out.printf("현재 시작은 \n %d시 %d분 %02d초 입니다.\n", 11, 30, 2); 
		System.out.printf("오늘 날짜는 %d년 %02d월 %d일 입니다.", 2022, 7, 28); // %04d => 0002
		// => %d 기호가 3개이므로, 데이터 3개가 순서대로 대입됨
		
		System.out.println(); // 줄바꿈
		// %f : 실수 표현
		// %.xf : 실수 표현(소수점 x 자리만큼만 표시)
		System.out.printf("평균 나이는 %.1f세 입니다.", 27.50);
		
		// %c : 문자 표현
		System.out.printf("정수 65에 해당하는 문자는 %c 입니다.\n", 'A');
		System.out.printf("정수 65에 해당하는 문자는 %c 입니다.\n", 65);
		
		// char (데이터)타입 변수 ch에 문자 'r' 저장
		char ch = 'r';
		System.out.printf("%c\n",ch);
		
		// %s : 문자열 표현
		// String 타입 변수 str에 문자열 "아이티윌 부산교육센터" 저장 // 대문자 S 로 시작하는 거 잊지말기
		String str = "아이티윌 부산교육센터";
		System.out.printf("교육기간명 : %s\n", str);
	}
