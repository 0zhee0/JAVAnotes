...
public class Ex {
	...
	public static void main(String[] args) {
		 ...
		 * 	 => 출발지 노드 -> 도착지 노드
		 * - 자바는 다양한 입출력 장치과 무관하고 일관성 있게 프로그램을 구현할 수 잇도록 
		 * 	 일종의 가상 통로인 스트림을 제공
		 * 
		 * [ 입력 스트림과 출력 스트림 ]
		 * 1. 입력 스트림
		 * => 자바에서 데이터가 입력될 때 처리하는 스트림
		 * => 예를들어 어떤 동영상을 재생하기 위해 동영상 파일에서 자료를 읽을 때
		 * => FileInputStream, FileReader, BufferedInputStream, BufferedReader 등
		 * 
		 * 2. 출력 스트림
		 * => 자바에서 데이터가 출력될 때 처리하는 스트림
		 * => 예를들어 편집 화면에서 사용자가 쓴 글을 파일에 저장할 때
		 ...
		 * 	=> InputStreamReader, OutputStreamWriterm BufferedInputStream, 
		 ...
		 * - 아무리 많은 데이터가 입력되어도 read() 메서드는 한 번에 1Byte만
		 * 	 처리할 수 있으므로 더 많은 데이터나 더 큰 단위 처리가 불가능
		 * 	 => 영문 또는 숫자 등의 데이터 1글자만 처리 가능
		 * 	 => 한글이나 한자 등 2Byte(char 단위) 문자들을 처리 불가능
		 * 	 => 읽어온 데이터가 int형이므로 문자로 변환 등의 후속 작업 필요
		 * - 가장 저수준의 입력 방법
		 * 
		 */
...
//		InputStream is = null;
//		// => finally 블록에서도 사용하기 위해 try문 위쪽에 전역변수로서 레퍼런스로(null값을 넣었기때문) 일단 선언
//		
//		try {
//			System.out.println("데이터를 입력하세요 : ");
//			is = System.in; // 로컬변수라서 finally문에서 is변수를 사용할 때 에러가 발생 그러므로 try문에서 벗어나 전역변수로 선언해야한다.
//							// 여기서는 전역변수 레퍼런스의 is를 객체로 만듦
//			// => System.in 코드에 의해 키보드로부터 데이터 입력이 가능하며
//			//	  입력스트림 객체를 InputStream 타입 변수에 저장(연결)
//			int n = is.read();
//			// => read() 메서드는 try ~ catch 문을 사용하여 예외 처리가 필수!
//			// => 입력스트림 데이터 중 1Byte만큼의 데이터를 읽어서 변수에 저장
...
//		}
		...
		try(InputStream is = System.in){
			...
			// 반복문을 사용하여 더 이상 읽어올 데이터가 없을 때(-1)까지 입력을 처리
			while(n != -1) {
