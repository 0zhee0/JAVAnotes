...
public class Ex {
	...
	public static void main(String[] args) {
		/*
		 * 중첩 if문
		 * - if 문(else if 또는 else 포함) 블록 내에서
		...
		if(score >= 0 && score <= 100) {
			...
			if(score >= 90) { // 90이상일 때 자동으로 100 이하가 적용됨
				grade = "A";
			} else if(score >= 80) { // 위의 조건이 거짓이면 자동으로 89이하 (80~89)
				grade = "B";				
			} else if(score >= 70) { // 위의 조건이 거짓이면 자동으로 79이하 (70~79)
				grade = "C";
			} else if(score >= 60) { // 위의 조건이 거짓이면 자동으로 69이하 (60~69)
				grade = "D";
			} else {
				// 위의 조건이 모두 거짓이면 자동으로 59이하이고,
				// 이미 첫번째 if 문의 조건식에서 0 ~ 100 사이 점수만 입력 가능하므로
				// 0 이상 59이하의 범위는 모두 else에 포함
				grade = "F";
			}		
