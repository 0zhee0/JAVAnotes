...
public class Ex {
	...
	public static void main(String[] args) {
		 ...
		 * 	 => 배열 형태이므로 배열 인덱스와 동일한 인덱스 번호르 사용
		...
		// equalsIgnoreCase() : 문자열 비교(대소문자 구별하지 않고 비교)
				System.out.println("s1.equals(JAVA PROGRAMMING) : " + 
										s1.equalsIgnoreCase("JAVA PROGRAMMING"));		// true
		...
		// => 5번 인덱스(P)부터 12번 인덱스(m) -1 한 (a)까지 추출 : "Program"
		
		// concat() : 문자열 결합(결합(연결, +)연산자보다 연산 속도가 빠름
		System.out.println("s1.concat(!) : " + s1.concat("!"));		// Java Programming!  // 속도 ↑
		System.out.println(s1 + "!");		// Java Programming!  // 속도 ↓
		
		// indexOf() : 특정 문자 또는 문자열의 인덱스 리턴
		// => 문자열의 앞쪽(첫번째 인덱스)부터 탐색
		// => 탐색할 문자 또는 문자열이 존재하지 않을 경우 -1 리턴
		System.out.println("s1.indexOf('a') : " + s1.indexOf('a'));		//  1 	// J 의 indexOf값은 0이다. 즉, 0부터 인덱스 시작. 1이 두 번째라는 의미.
		// => 앞에서부터 문자 a를 탐색하여 처음 만나는 인덱스 1 리턴
		
		System.out.println("s1.indexOf('x') : " + s1.indexOf('x'));		// -1
		// => 존재하지 않는 문자이므로 -1 리턴
		...
		//		" Java Programming" -> "J@V@ Programming"
		System.out.println("s1.replace('a', '@') : " + s1.replace("a", "@"));		// J@v@ Progr@mming
		// 	=> char 타입 대신 String 타입 문자열도 지정 가능
		System.out.println("s1.replace(Java, Android) : " + s1.replace("Java", "Android"));		// Android Programming
