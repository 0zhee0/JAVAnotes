 ...
 * - (넓은) public > potected > default > private (좁음)
 * 	 => 단, default 접근제한자는 별도의 키워드 없이 생략하면 적용
 * 
 * 1. private 접근제한자
 * - 외부 클래스에서 접근하지 못하여, 자신의 클래스 내에서만 접근 가능
 ...
 * 		 ex) 멤버변수 num에 접근하여 Getter 메서드 이름 : getNum() 
 * 	  => 파라미터는 없고, 리턴값만 있는 메서드
 * 		 ex) public int getNum() {}
 * 
 * 2. Setter : 외부로부터 값을 전달받아 내부 멤벼변수에 저장하는 메서드	
 * 	  => setXXX() 형식의 메서드 이름을 사용하여 정의
 * 		 이때, XXX 는 접근할 멤버변수의 이름을 사용
