...
//		System.out.println("static 메서드에서 인스턴스 변수 접근 : " + normalVar);
		...
		// => 인스턴스 변수는 아직 메모리에 로딩되지 전이므로 접근 불가능!
		System.out.println("static 메서드에서 인스턴스 변수 접근 : " + staticVar);
		
		// static 메서드에서 일반 메서드 호출 불가(변수의 원인과 동일함)
//		normalMethod();
		// Cannot make a static reference 
		// to the non-static method normalMethod() from the type StaticMethod
