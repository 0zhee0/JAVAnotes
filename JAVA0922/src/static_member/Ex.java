...
public class Ex {
	...
	public static void main(String[] args) {
		 ...
		 * 	 인스턴스 생서과 상관없이 클래스가 로딩되는 시점에 메모리에 로딩됨
		 ...
		 * 
		 * < static 변수 >
		 * - 멤버변수 static 키워드를 사용할 경우 클래스(정적) 멤버로 취급됨
		 * - 인스턴스 생성 전, 클래스가 메모리에 로딩될 때 함께 로딩됨
		 * 	 => 모든 인스턴스에서 하나의 변수를 공유함
		 * 	 => 즉, 클래스 당 하나만 생성됨(클래스 영역 = Method Area에 생성됨)
		 * 	 => 인스턴스 생성 없이도 사용 가능
		 * - 클래스명.멤버변수명 형태로 접근
		 */
...
//		System.out.println("NormalMember : " + NormalMember.a);
		...
		// => Static 이 정의되어 있지 않은 인스턴스 멤버변수 a에 접근하려고 하면
		//	  에러가 발생한다!
		...
		// n2 인스턴스의 인스턴스 멤버변수 b값을 999로 변경
		n1.a = 99;
		n1.b = 999;
		...
		System.out.println("StaticMember : " + StaticMember.a);
		// => 인스턴스가 생서되기 전이지만 이미 메모리에 로딩되어 있으므로 
		...
		StaticMember.a = 500;
//		StaticMember.b = 500; // 컴파일에러 발생! 클래스명만으로는 접근 불가능
		System.out.printf("s1.a : %d, s2.a : %d \n", s1.a, s2.a);
		
		System.out.println("=================================================");
		
		// Static 멤버변수의 대표적인 예
		// => java.lang.Meth 클래스의 PI 변수
