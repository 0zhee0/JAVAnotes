...
class Child extends Parent{
	...
	// 슈퍼클래스로부터 상속받은 watchTv() 메서드 오버리딩
	...
	public void watchParentTv() {
		...
		// 레퍼런스 super를 통해 접근화면 사요잉 가능하다!
		// => 레퍼런스 this와 사용 방법이 동일함
		
		// 1. 슈퍼클래스의 은늑된 벱버변수에 접근할 때
...
//	String x = "Child"; // 서브클래스의 멤버변수(오버라이딩)
	...
	public void method() {
		...
		// => 만약, 로컬변수를 선언하지 않을 경우 this.x와 동일
		
		System.out.println("this.x = " + this.x); //서브클래스 멤버변수 x
		// => 만약, Child 클래스 멤버변수를 선언하지 않을 경우 super.x와 동일
