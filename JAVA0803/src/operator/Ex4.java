...
public class Ex4 {
	...
	public static void main(String[] args) {
		 ...
		 * - 다른 연산과 결합하여 사용될 때 위치에 따라 결과가 다르다.
		 * 
		 * 1. 선행연산자(전위연산자)
		 * - 피연산자 앞(좌측)에 위치하여 값을 1만큼 증가 또는 감소시킨다.
		 *   이 때, 다른 연산과 결합하여 사용하는 경우
		 *   먼저 피연산자의 값을 1 증가 또는 감소시킨 후 다른 연산에 참여한다.
		 *   ex) ++a, --a
		 * 
		 * 2. 후행연산자(후위연산자)
		 * - 피연산자 뒤(우측)에 위치하여 값을 1만큼 증가 또는 감소시킨다.
		 *   이 때, 다른 연산과 결합하여 사용하는 경우
		 *   먼저 다른 연산에 참여시킨 후 피연산자의 값을 1증가 또는 감소 시킨다.
		 *    ex) a++, a--
		 * 
		 */
...
//		a = a + 1; // 컴파일에러 발생! (byte + int = int + int = int 이므로 저장 불가)
		a = (byte)(a + 1); // 형변환 필요
		...
		b--;
		System.out.println(b); //변수 b의 값을 1만큼 감소시킴 7 -> 6 으로 감소
		
		--b;
		System.out.println(b); //변수 b의 값을 1만큼 감소시킴 6 -> 5 로 감소
		...
		//    대입연사자를 통해 x1 값을 y1 에 대입(저장)
		// => 따라서, x1 의 값이 5 -> 6으로 증가한 후에 y1에 증가된 값 6이 저장됨
		...
		// => 따라서, x2 값 5가 먼저 y2 에 저장된 후, x2 값이 5 -> 6 으로 증가
		...
		System.out.println(x2); // x2 출력 시 7 출력
		...
		// ch 값을 직접 변경하지 않았으므로
		System.out.println(ch); // 변경 전인 'A' 출력됨
...
//		System.out.println(ch++);
		...
		int y3 = 20;
		int z3 = ++x3 + y3--;
		System.out.println(z3); // 11 + 20 = 31
